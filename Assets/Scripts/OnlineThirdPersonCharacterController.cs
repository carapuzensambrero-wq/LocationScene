...
/// <summary>
/// Basic FishNet-ready version of ThirdPersonCharacterController.
///
/// This controller is intentionally owner-authoritative for the first multiplayer test:
/// the owning client reads local input and moves its CharacterController, while FishNet's
/// NetworkTransform component on the same prefab synchronizes the transform to observers.
/// </summary>
[RequireComponent(typeof(CharacterController))]
[RequireComponent(typeof(NetworkObject))]
[RequireComponent(typeof(NetworkTransform))]
public sealed class OnlineThirdPersonCharacterController : NetworkBehaviour
{
    ...
    private void SetLocalControls(bool enabled)
    {
        ...
        if (crosshairUI != null)
            crosshairUI.gameObject.SetActive(enabled);
    ...
    private void HandleFireInput()
    {
        ...
        {
            // Local visual feedback only for now. Networked weapons can be added later via ServerRpc.
    ...
    private void MoveCharacter()
    {
        ...
        if (facing.sqrMagnitude > 0.001f)
        {
            ...
            float rotationT = aimBlend > 0.5f
                ? 1f - Mathf.Exp(-aimRotationSharpness * Time.deltaTime)
                : 1f - Mathf.Exp(-rotationSpeed * 4f * Time.deltaTime);
